...
const renderFavouriteMovies = async function (data) {
  ...
  if (data && data.length) {
    ...
    data.forEach((movie, index) => {
      ...
      content.innerHTML += `
                    <div class="swiper-slide" onclick='goFilmDetailPage("${
                      movie.id
                    }")'>
                      <div class="slide-content">
                        ...
                        <div class="slide-overlay">
                          <div class="slide-info">
                            ...
                            }">${movie.category?.name || ""}</span>
                            <div class="rating">${"★".repeat(star)}</div>
                            <h4 class="title">${movie.title}</h4>
                          </div>
                          <a class="watch-now" target="_blank" onclick='${
                            movie.watch_url
                          }'>Watch Now ></a>
                        </div>
                      </div>
    ...
    content.innerHTML = "Favourite movies is not available";
  }
};
